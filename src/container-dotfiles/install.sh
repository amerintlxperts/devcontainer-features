...
su -l "${_REMOTE_USER}" -c "echo 'gh auth status | grep -q 'Logged in to github.com account' || gh auth login' >> ${_REMOTE_USER_HOME}/.bashrc"
su -l "${_REMOTE_USER}" -c "echo 'gh auth status | grep -q 'Logged in to github.com account' || gh auth login' >> ${_REMOTE_USER_HOME}/.zshrc"
